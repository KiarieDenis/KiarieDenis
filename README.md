#  Hi, I'm Denis Kiarie

**`Data Scientist | ML/AI Engineer | Systems Thinker`**

I build reliable, scalable systems through automation and data-driven approaches. Rather than chasing trends, I study patterns and focus on tools that create long-term value.

   <p align="left">
      <a href="https://github.com/KiarieDenis?tab=followers">
         <img alt="followers" title="Follow me on GitHub" src="https://custom-icon-badges.demolab.com/github/followers/KiarieDenis?color=236ad3&labelColor=1155ba&style=for-the-badge&logo=person-add&label=Follow&logoColor=black"/>
      </a>
      <a href="https://github.com/KiarieDenis?tab=repositories&sort=stargazers">
         <img alt="total stars" title="Total stars on GitHub" src="https://custom-icon-badges.demolab.com/github/stars/KiarieDenis?color=55960c&style=for-the-badge&labelColor=black&logo=star"/>
      </a>
   </p>

---

### Tech Stack

![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=black)
![Pandas](https://img.shields.io/badge/pandas-%23150458.svg?style=for-the-badge&logo=pandas&logoColor=black)
![scikit-learn](https://img.shields.io/badge/scikit--learn-%23F7931E.svg?style=for-the-badge&logo=scikit-learn&logoColor=black)
![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=for-the-badge&logo=fastapi)
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=black)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

**Learning:** Machine Learning, Deep Learning, Game Development

---

### Core Philosophy

> *Most people learn tools. I study patterns.*

I connect ideas across domains – from ML pipelines to backend systems and automation platforms. My focus is on **leverage**: systems that scale effort, eliminate manual work, and compound over time.

- **Vision:** Intelligent platforms, not one-off scripts
- **Goal:** AI that augments decision-making
- **Approach:** Build before talking. Optimize for compounding skills.

---

### What I'm Working On

| Area | Focus |
|------|-------|
| **Data Science** | Pattern recognition, predictive analytics |
| **ML/AI** | Recommendation systems, prediction models, automation agents |
| **Systems Thinking** | Scalable data pipelines, API design |

**Recent example:** Built a prediction model processing 200+ matches across 15 leagues – identifying value patterns using only form data and odds. (Full project coming soon)

---

### What I'm Looking For

| Role Type | Interest |
|-----------|----------|
| **Data Analyst / Scientist** | Predictive modeling, pattern discovery |
| **ML/AI Engineer** | Applied AI, automation agents |
| **Collaboration** | Open-source data projects, systems thinking discussions |

---

### Find Me

- **Portfolio:** [denis-kiarie-porfolio.onrender.com](https://denis-kiarie-porfolio.onrender.com)
- **GitHub:** [github.com/KiarieDenis](https://github.com/KiarieDenis)
- **Email:** kangethedennis34@gmail.com

---

*Based in Kenya | Building intelligent systems that scale*
